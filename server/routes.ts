...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/driver-activity/driver/:driverName/:year/:month", async (req, res) => {
    try {
      ...
      const activity = await getDriverDetailedActivity(decodeURIComponent(driverName), parseInt(year), parseInt(month));
      res.json(activity);
    } catch (error) {
  ...
  /**
   * Analizează activitatea șoferilor pentru o lună specifică
   */
  async function analyzeDriverActivityForMonth(year: number, month: number) {
    console.log(`📊 Analyzing driver activity for ${month}/${year}`);
    
    // Obține toate procesările săptămânale pentru luna specificată
    const weeklyData = await storage.getAllWeeklyProcessing();
    
    // Filtrează datele pentru luna și anul specificate
    ...
    const driverActivity: Record<string, any> = {};

    // Procesează fiecare săptămână
    for (const week of monthlyWeeks) {
      ...
      // Extrage informații despre șoferi din datele procesate
      Object.keys(processedData).forEach(companyName => {
        if (companyName === 'Unmatched' || companyName === 'Totals') return;

        const companyData = processedData[companyName];
        
        // Caută șoferii activi în datele companiei
        if (companyData.drivers) {
          Object.keys(companyData.drivers).forEach(driverName => {
            ...
            // Verifică dacă șoferul a fost activ (cel puțin 1 cursă)
    ...
    // Calculează zilele de muncă și odihnă pentru fiecare șofer
    ...
    Object.keys(driverActivity).forEach(driverName => {
      ...
      // Estimează zilele de muncă bazat pe săptămânile active
      // Presupunem că o săptămână activă = 5-6 zile de muncă
    ...
    return {
      ...
      drivers: Object.values(driverActivity)
        .sort((a, b) => b.totalTrips - a.totalTrips), // Sortează după numărul de curse
      summary: {
  ...
  /**
   * Obține activitatea detaliată pentru un șofer specific
   */
  async function getDriverDetailedActivity(driverName: string, year: number, month: number) {
    console.log(`🔍 Getting detailed activity for ${driverName} in ${month}/${year}`);
    ...
    const driverDetails = {
      ...
      summary: {
        ...
        bestWeek: null as string | null,
        worstWeek: null as string | null
      }
    };
    ...
    for (const week of monthlyWeeks) {
      ...
      // Caută șoferul în datele procesate
      Object.keys(processedData).forEach(companyName => {
        if (companyName === 'Unmatched' || companyName === 'Totals') return;
        ...
        if (companyData.drivers && companyData.drivers[driverName]) {
          ...
          weekActivity.isActive = weekActivity.tripCount >= 1;
          ...
          if (weekActivity.isActive) {
            ...
            // Urmărește cea mai bună și cea mai slabă săptămână
    ...
    if (driverDetails.summary.totalActiveWeeks > 0) {
      driverDetails.summary.averageTripsPerWeek = 
        (driverDetails.summary.totalTrips / driverDetails.summary.totalActiveWeeks);
  ...
  /**
   * Obține sumar anual pentru toți șoferii
   */
  async function getYearlyDriverActivitySummary(year: number) {
    console.log(`📈 Getting yearly summary for ${year}`);
    ...
    for (let month = 1; month <= 12; month++) {
      ...
      monthlySummaries.push({
        ...
        mostActiveDriver: monthlyActivity.summary.mostActiveDriver.name || 'N/A'
      });
    }
  ...
  /**
   * Parsează label-ul săptămânii pentru a extrage informații despre dată
   */
  function parseWeekLabel(weekLabel: string) {
    // Extract year from label like "10 aug. 2025 - 16 aug. 2025"
    const yearMatch = weekLabel.match(/(\d{4})/);
    if (!yearMatch) {
      console.warn(`Could not parse year from week label: ${weekLabel}`);
      return { year: 0, month: 0, weekNumber: 0 };
    }

    const year = parseInt(yearMatch[1]);
    
    // Extract month from Romanian month names
    ...
    let month = 1;
    for (const [monthAbbr, monthNum] of Object.entries(monthMap)) {
      if (weekLabel.toLowerCase().includes(monthAbbr)) {
        month = monthNum;
        break;
      }
    }

    // Estimate week number based on date
    const weekNumber = Math.ceil(month * 4.33);
  ...
  /**
   * Calculează zilele lucrătoare dintr-o lună (exclud weekend-urile)
   */
